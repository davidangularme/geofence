    ...
    override fun onReceive(context: Context, intent: Intent) {
        if (intent.action == ACTION_GEOFENCE_EVENT) {
            ...
            if (geofencingEvent.geofenceTransition == Geofence.GEOFENCE_TRANSITION_EXIT) {
                    ...
                    (( HuntMainActivity().getInstance()!!.getLastLocation()))
                Log.v(TAG, context.getString(R.string.geofence_over))
                ...
                // Check geofence against the constants listed in GeofenceUtil.kt to see if the
                // user has go outside any of the locations we track for geofences.
                val foundIndex = GeofencingConstants.LANDMARK_DATA.indexOfFirst {
                    it.id == fenceId
                }
                ...
                notificationManager.sendGeofenceExitNotification(
                    context, foundIndex
                )
            }
        }
